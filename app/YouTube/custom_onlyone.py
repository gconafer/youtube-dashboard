...
def export_onlyone(request, year_month, client_id):
  ...
  sound_recording = pd.read_sql_query(f"""
    select asset_id,
           ...
           owned_views,
           partner_revenue
    from (select asset_id, coalesce(partner_revenue, 0) + coalesce(promotion_partner_revenue, 0) as partner_revenue, coalesce(owned_views, 0) + coalesce(promotion_owned_views, 0) as owned_views
          from (select asset_id, sum(owned_views) as owned_views, sum(partner_revenue) as partner_revenue
                ...
                group by asset_id) O
                   full outer join (select asset_id, sum(split_partner_revenue) as promotion_partner_revenue, sum(split_onwed_views) as promotion_owned_views
                                    from (
  ...
  folder_id = '1O77oSrrsuafDyjVrRESsn57XTwAVEKtv'
  ...
  update_request.append({
    'range': f'G{3 + length}:H{4 + length}',
    'values': [[f'=SUM(G3:G{2 + length})', f'=SUM(H3:H{2 + length})'],
               ['', f'=SUM(H3:H{2 + length})*{float(client.sr_split)}']],
  })
  ...
  update_request.append({
    'range': f'G{3 + length}:H{4 + length}',
    'values': [[f'=SUM(G3:G{2 + length})', f'=SUM(H3:H{2 + length})'],
               ['', f'=SUM(H3:H{2 + length})*{float(client.at_split)}']],
  })
  ws.batch_update(update_request, value_input_option='USER_ENTERED')
  batch_update_request = format_srat(batch_update_request, length, ws)
  ...
  if len(mc_revenues) > 0:
    ...
    update_request["mc"].append({
      'range': f'G{3 + length}:H{4 + length}',
      'values': [[f'=SUM(G3:G{2 + length})', f'=SUM(H3:H{2 + length})'],
                 ['', f'=SUM(H3:H{2 + length})*{float(client.mc_split)}']],
    })
  ...
  item_list = [
    [f"='온리원뮤직ONLYONEMUSIC'!C{length[1] + 4}"],
    [f"='음원'!H{length[2] + 4}"],
    [f"='아트트랙'!H{length[3] + 4}"],
  ]
  if len(mc_revenues) > 0:
    item_list.append([f"='직접소유권주장'!H{length[4] + 4}"])
