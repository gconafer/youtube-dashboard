...
def export_leeway(request, year_month, client_id):
  ...
  update_request.append({
    'range': f'G{3 + length}',
    'values': [[f'=SUM(G3:G{2 + length})'],
               [f'=SUM(G3:G{2 + length})*{float(client.sr_split)}']],
  })
  ...
  if len(mc_revenues) > 0:
    ...
    update_request["mc"].append({
      'range': f'G{3 + length}',
      'values': [[f'=SUM(G3:G{2 + length})'],
                 [f'=SUM(G3:G{2 + length})*{float(client.mc_split)}']],
    })
